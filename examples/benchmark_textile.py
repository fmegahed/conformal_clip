...
def main():
    ...
    tr_nom, tr_def, calib, calib_labels, test, test_labels = build_textile_splits()
    # We'll let each backend preprocess internally in benchmark_models; here we keep PILs
    train_nominal_images = load_pil(tr_nom)
    train_defective_images = load_pil(tr_def)
    calib_images = load_pil(calib)
    test_images = load_pil(test)
    ...
    cls_df, cp_df, cls_style, cp_style = benchmark_models(
        ...
        alpha_list=(0.1,),
        allow_empty=False,
        csv_path="results/textile",
        csv_prefix="textile_"
    )

    os.makedirs("results", exist_ok=True)
    cls_df.to_csv("results/textile_benchmark_classification.csv", index=False)
    cp_df.to_csv("results/textile_benchmark_conformal.csv", index=False)
    cls_style.to_html("results/textile_benchmark_classification.html")
    cp_style.to_html("results/textile_benchmark_conformal.html")
