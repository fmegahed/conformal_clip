...
from _shared_pipe import build_pipe_splits, load_pil_from_github
...
def main():
    ...
    tr_nom, tr_def, calib, calib_labels, test, test_labels = build_pipe_splits()
    ...
    cls_df, cp_df, cls_style, cp_style = benchmark_models(
        ...
        alpha_list=(0.1,),
        allow_empty=False,
        csv_path="results/pipe",
        csv_prefix="pipe_"
    )

    os.makedirs("results", exist_ok=True)
    cls_df.to_csv("results/pipe_benchmark_classification.csv", index=False)
    cp_df.to_csv("results/pipe_benchmark_conformal.csv", index=False)
    cls_style.to_html("results/pipe_benchmark_classification.html")
    cp_style.to_html("results/pipe_benchmark_conformal.html")
