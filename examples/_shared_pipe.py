"""
Shared Utilities for Pipe Staple Examples
=========================================

This module provides shared data loading and splitting utilities for the pipe
staple examples. 

Functions:
    build_pipe_splits: Create reproducible train/calibration/test splits
...
def build_pipe_splits(seed: int = 2025) -> Tuple[
    List[str], List[str], List[str], List[str], List[str], List[str]
]:
    """Return (train_nominal, train_defective, calib, calib_labels, test, test_labels) image paths.

    Keeps class balance simple: uses missing as defective.
    """
    repo_owner = "fmegahed"
    repo_name = "qe_genai"
    base_path = "data/pipe_staple/test"
    
    rng = np.random.default_rng(seed)

    nom = get_image_urls(repo_owner, repo_name, base_path, "good")
    nom.sort()
    defect = get_image_urls(repo_owner, repo_name, base_path, "missing")
    defect.sort()
    ...
    # Test set
    test_nom = sample(nom, 27)
    test_def = sample(defect, 27)
    ...
    def_left = [p for p in defect if p not in test_def]

    # Train exemplars (few-shot banks)
    tr_nom = sample(nom_left, 50)
    tr_def = sample(def_left, 50)

    # Calibration set
    nom_left2 = [p for p in nom_left if p not in tr_nom]
    def_left2 = [p for p in def_left if p not in tr_def]
    cal_nom = sample(nom_left2, 40)
    cal_def = sample(def_left2, 40)
